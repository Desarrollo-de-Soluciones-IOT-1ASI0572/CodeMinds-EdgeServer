...
class TrackingRecordApplicationService:
    ...
    def create_tracking_record_with_backend(self, device_id: str, rfid_code: str, api_key: str, latitude: float, longitude: float, speed: float = 0, created_at: str = None) -> TrackingRecord:
        ...
        try:
            ...
            record = self.tracking_service.create_record(device_id, latitude, longitude, speed, created_at)
            ...
            # If backend post fails, delete the local record and re-raise the exception
            self.tracking_repository.delete(saved_record.id)
            raise ValueError(f"Backend post failed, local record deleted: {str(e)}")

    def create_tracking_record(self, device_id: str, api_key: str, latitude: float, longitude: float, speed: float, created_at: str) -> TrackingRecord:
    ...
    def get_locations_by_device(self, device_id: str, api_key: str) -> list[TrackingRecord]:
